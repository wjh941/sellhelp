...
test('reports maps categories from loaded products, guards CSV formulas, and excludes history from print', async () => {
  ...
  assert.match(source, /const productPageSize\s*=\s*100/)
  assert.match(source, /const maxProductPages\s*=\s*100/)
  assert.match(source, /while \(page <= maxProductPages\)/)
  assert.match(source, /getProducts\(\{ page, page_size: productPageSize \}\)/)
  assert.match(source, /loadedProducts\.length >= total/)
  assert.match(source, /if \(!items\.length\) break/)
  assert.match(source, /const categoryMapWarning\s*=\s*ref\(''\)/)
  assert.match(source, /v-if="categoryMapWarning"/)
  assert.match(source, /page > maxProductPages && loadedProducts\.length < total/)
  assert.match(source, /品类筛选可能不完整/)
