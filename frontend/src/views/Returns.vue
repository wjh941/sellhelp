...
import { confirmStockTake as confirmStockTakeApi, createReturn, getProductStock, getProducts, getReturns, prepareStockTake } from '@/api'
...
const returnForm = reactive({ return_type: customerReturnType, related_order_no: '', product_id: null, batch_id: null, quantity: 0, refund_amount: 0, reason: '', operator: '' })
...
const variance = row => Number(row.actual_quantity || 0) - Number(row.system_quantity || 0)
...
function openCreateDialog(type) {
  ...
  Object.assign(returnForm, { return_type: type, related_order_no: '', product_id: null, batch_id: null, quantity: 0, refund_amount: 0, reason: '', operator: '' })
...
async function confirmStockTakeAction() {
  try {
    ...
    const items = stockTakeItems.value.map(item => ({ product_id: item.product_id, batch_id: item.batch_id, actual_quantity: item.actual_quantity, reason: '' }))
...
onMounted(async () => { await loadProducts(); loadReturns() })
</script>
