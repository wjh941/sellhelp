...
Windows 桌面版在程序运行期间每小时检查一次；距离上次成功自动备份达到 24 小时时，会在
`%LOCALAPPDATA%\SellHelp\backups` 创建一份 SQLite 在线备份，并只保留最新 14 份自动备份。
手动备份、恢复前备份和迁移保护备份不会被自动清理。自动备份失败不会中断业务，设置页会显示最近失败状态；
请检查磁盘空间并保持程序运行，系统会在下一次检查时重试。

自动备份只保存在本机，不能防止电脑丢失、磁盘损坏或勒索软件。应定期通过设置页下载手动备份，
并复制到外接盘或受控网络存储。

Windows 桌面版 owner 还可在“数据库备份”的“异地副本”区域选择一个现有目录。系统会在本地自动备份
完成后复制最新副本到该目录，并在目标目录中仅保留最新 14 份自动副本；不会删除该目录中的其他文件。
应选择外接盘、NAS 映射盘或同步盘目录，不能选择与应用数据相同的物理磁盘。目标目录暂时不可用时，
本地备份和业务不会中断；设置页会显示失败状态，程序运行期间每小时自动重试。停用异地副本只移除配置，
不会删除已经复制的文件。
